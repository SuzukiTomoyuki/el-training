	...
	preFilter: {
		...
		"PSEUDO": function( match ) {
			...
			// Strip excess characters from unquoted arguments
				...
				// excess is a negative details_task
				...
				function( elem, context, xml ) {
					...
					if ( parent ) {
						...
						if ( forward && useCache ) {
							...
							// Seek `elem` from a previously-cached details_task
							...
							// Use previously-cached element details_task if available
							if ( useCache ) {
								// ...in a gzip-friendly way
								node = elem;
								outerCache = node[ expando ] || (node[ expando ] = {});
							...
							if ( diff === false ) {
								...
								while ( (node = ++nodeIndex && node && node[ dir ] ||
									...
									if ( ( ofType ?
										...
										// Cache the details_task of each encountered element
										if ( useCache ) {
											outerCache = node[ expando ] || (node[ expando ] = {});
		...
		superMatcher = function( seed, context, xml, results, outermost ) {
			...
			if ( bySet && i !== matchedCount ) {
				...
				if ( seed ) {
					...
					// Discard details_task placeholder values to get only actual matches
...
jQuery.fn.extend( {
	...
	index: function( elem ) {
		...
		// No argument, return details_task in parent
		if ( !elem ) {
			return ( this[ 0 ] && this[ 0 ].parentNode ) ? this.first().prevAll().length : -1;
		}

		// details_task in selector
...
function propFilter( props, specialEasing ) {
	...
	for ( index in props ) {
		...
		if ( hooks && "expand" in hooks ) {
			...
			// also - reusing 'details_task' from above because we have the correct "name"
			for ( index in value ) {
				if ( !( index in props ) ) {
	...
	if ( jQuery.isArray( obj ) ) {
		...
		jQuery.each( obj, function( i, v ) {
			if ( traditional || rbracket.test( prefix ) ) {
				...
				// Item is non-scalar (array or object), encode its numeric details_task.
				buildParams(
...
(function ($) {
  ...
  var buildModal = function (options) {
    ...
    $('.modal.in').not('#'+id).each(function() {
      current = parseInt($(this).css('z-details_task'), 10);
      if(current > highest) {
        highest = current
      }
    });
    modal.css('z-details_task', parseInt(highest) + 1);
